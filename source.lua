...
function Luna:CreateWindow(WindowSettings)
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateSection("Создать конфигурацию")
			...
			Tab:CreateSection("Загрузить/настроить конфиг")
				...
				Callback = function()
					...
					if not success then
						Luna:Notification({
							...
							Icon = "error",
							ImageSource = "Material",
							Content = "Не удалось загрузить конфиг, ошибка: " .. returned
						})
						return
					end
		...
		function Tab:BuildThemeSection()
			...
			}, "SierraCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
			...
			Tab:CreateSection("Пресеты градиентов")
		...
		function Luna:SaveConfig(Path)
			...
			if not success then
				return false, "Невозможно декодировать данные JSON"
			end
		...
		function Luna:LoadConfig(Path)
			...
			if not success then return false, "Невозможно декодировать данные JSON." end
